      ...
      <div class="col-12">
        ...
        @if ($data->count() > 0)
          @if ($canExport)
            <div class="card">
              <div class="card-header">
                <h4><i class="fa fa-fas fa-users"></i> {!! __('Aksi Ekspor <small>(Server Side)</small>') !!}</h4>
                <div class="card-header-action">
                    ...
                    @foreach ($data as $item)
                      <tr>
                        ...
                        <td>
                          ...
                          @endif
                          @if ($canDelete)
                            @include('stisla.includes.forms.buttons.btn-delete', ['link' => route('kelompoks.destroy', [$item->id])])
                          @endif
                        </td>
                      </tr>
                    @endforeach
        ...
        @else
          @include('stisla.includes.others.empty-state', ['title' => 'Data ' . $title, 'icon' => 'fas fa-users', 'link' => $routeCreate])
        @endif
      </div>
